...
def check_breathing(breathing, errors, warnings):
    ...
    # Продължителност и дълбочина не са едно и също. Инструкцията „дишай бавно" често
    # кара човек да диша по-дълбоко, а по-дълбокото дишане сваля CO2 — обратното на
    # търсеното. Протокол, който мълчи за дълбочината, оставя това на случайността.
    if not any(p.get("depth") for p in phases if isinstance(p, dict)):
        warnings.append("нито една фаза не казва нищо за дълбочината на дишането — "
                        "задача 6.1 трябва да я определи")
